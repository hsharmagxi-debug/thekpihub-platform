...
// ── Color helpers ─────────────────────────────────────────────
const sc = s => s==='green'?'#4CAF80':s==='yellow'?'#E9A123':'#E05C5C';
const sv = s => s==='High'?'#E05C5C':s==='Medium'?'#E9A123':'#4CAF80';
const hc = n => n>=75?'#4CAF80':n>=50?'#E9A123':'#E05C5C';
...
function renderResults(r) {
  ...
  set('audit-score', `
    <div style="text-align:center;padding:2.5rem 1rem;background:rgba(255,255,255,.03);border:1px solid rgba(233,161,35,.2);border-radius:12px;margin-bottom:1.5rem">
      <div style="font-size:5rem;font-weight:800;color:${hc(r.health_score)};font-family:'Cormorant Garamond',serif;line-height:1">${r.health_score}</div>
  ...
  set('audit-metrics', `
    ...
    <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(250px,1fr));gap:.7rem">
      ${r.metrics.map(m=>`
        <div style="background:rgba(255,255,255,.04);border:1px solid rgba(233,161,35,.1);border-left:3px solid ${sc(m.status)};border-radius:8px;padding:1rem 1.1rem">
          <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:.3rem">
            ...
            <span style="color:${sc(m.status)};font-weight:700;font-size:1rem;font-family:'Cormorant Garamond',serif">${m.value}</span>
          </div>
  ...
  set('audit-cohort', `
    <h3 style="color:#E9A123;font-size:.78rem;text-transform:uppercase;letter-spacing:.2em;margin-bottom:1rem;font-family:Syne,sans-serif">Peer Cohort Ranking</h3>
    <div style="text-align:center;padding:1.5rem;background:rgba(233,161,35,.06);border:1px solid rgba(233,161,35,.2);border-radius:10px">
      <div style="font-size:2.2rem;font-weight:700;color:#E9A123;font-family:'Cormorant Garamond',serif">${r.cohort_rank}</div>
      <div style="color:#EAEDF5;opacity:.6;font-size:.88rem;margin-top:.5rem">${r.cohort_insight}</div>
    </div>`);

  set('audit-risks', `
    <h3 style="color:#E9A123;font-size:.78rem;text-transform:uppercase;letter-spacing:.2em;margin-bottom:1rem;font-family:Syne,sans-serif">Top Risks & Actions</h3>
    ${r.top_risks.map(risk=>`
      <div style="background:rgba(255,255,255,.03);border:1px solid rgba(233,161,35,.1);border-radius:8px;padding:.9rem 1.1rem;margin-bottom:.6rem">
        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:.3rem">
          <span style="color:#EAEDF5;font-weight:600;font-size:.9rem">${risk.title}</span>
          <span style="background:${sv(risk.severity)}22;color:${sv(risk.severity)};font-size:.7rem;padding:.15rem .55rem;border-radius:20px;font-weight:700;text-transform:uppercase;letter-spacing:.08em">${risk.severity}</span>
        </div>
        <div style="color:#E9A123;font-size:.8rem;opacity:.8">→ ${risk.action}</div>
      </div>`).join('')}`);

  set('audit-wins', `
    <h3 style="color:#E9A123;font-size:.78rem;text-transform:uppercase;letter-spacing:.2em;margin-bottom:1rem;font-family:Syne,sans-serif">Quick Wins</h3>
    <ul style="list-style:none;padding:0;margin:0">
      ${r.quick_wins.map(w=>`
        <li style="color:#EAEDF5;font-size:.88rem;padding:.6rem 0;border-bottom:1px solid rgba(255,255,255,.05);display:flex;gap:.75rem;align-items:flex-start">
          <span style="color:#4CAF80;flex-shrink:0">✓</span>
          <span style="opacity:.75;line-height:1.5">${w}</span>
        </li>`).join('')}
    </ul>`);
}
...
document.addEventListener('DOMContentLoaded', () => {
  // Wire button — try multiple selectors
  const btn = document.querySelector('#run-audit, .run-audit, [data-action="audit"]')
           || [...document.querySelectorAll('button')].find(b =>
                b.textContent.toLowerCase().includes('audit') ||
                b.textContent.toLowerCase().includes('run'));
  if (btn) { btn.id = 'audit-btn'; btn.onclick = null; btn.addEventListener('click', runAudit); }

  // Inject result placeholders + loading div
  const wrap = document.querySelector('main, .container, #app, body');
  ...
  if (!document.getElementById('audit-loading')) {
    const ld = Object.assign(document.createElement('div'), { id:'audit-loading' });
    ld.style.cssText = 'display:none;text-align:center;padding:2rem;color:#E9A123;font-family:Syne,sans-serif;font-size:.9rem;letter-spacing:.1em;animation:pulse 1.5s infinite';
    ld.textContent = '⟳  Claude is auditing your KPIs — this takes ~10 seconds';
    wrap?.appendChild(ld);
  }
  // Pulse animation
  if (!document.getElementById('audit-pulse-style')) {
    const style = document.createElement('style');
    style.id = 'audit-pulse-style';
...
def patch_file(filepath):
    ...
    if 'kpihub_anthropic_key' in content:
        print(f"  [SKIP]    {filepath} — AI engine already present")
        return False

    if '</body>' not in content:
        print(f"  [WARN]    {filepath} — no </body> tag found")
        return False
...
if __name__ == '__main__':
    ...
    if not os.path.exists(target):
        ...
        if ok:
            print("\n" + "=" * 55)
            print("  ✅ auditor.html — Claude AI wired successfully")
            print("=" * 55)
            print("""
  Next steps:
  1. git add auditor.html
  2. git commit -m "feat: wire Claude AI into KPI Auditor"
  3. git push origin main
  4. Visit thekpihub.com/auditor.html
  5. Fill in your KPIs → Run Full KPI Audit
  6. Enter Anthropic key when prompted (saved locally)
""")
